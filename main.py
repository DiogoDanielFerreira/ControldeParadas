...
# Constantes
#DATA_FILE = r"C:\Users\ferreid1\KWS SAAT SE\PPCM - Documentos\General\Controle de Paradas\ControleParadas.xlsx"
SHEET_NAME = "Paradas"
...
def filter(df, st):
    ...
    setor = st.sidebar.multiselect("Entre com os setores", df['SetorCausador'].unique(), ["Automacao", "Eletrica", "Mecanica"])
    df_filtered = df.query("Data >= @start_date and Data <= @end_date and SetorCausador == @setor")
...
def main():
    ...
    # Qtde engeman
    df_engeman = df_engeman.query("Data >= @start_date and Data <= @end_date and (setor == 'RC01' or setor == 'UB02' or setor == 'RC02' or setor == 'DB01' or setor == 'RG01')")
    qtde = df_engeman["Data"].count()
    
    # Titulo da pagina
    st.title("Daily Report Winter 24' - Stop Control")
    # Texto
    st.subheader(f"Data inicial: :blue[{start_date}] - Data final: :blue[{end_date}]")
...
# Menu de opções
carregar_dados, graficos, tabelas, quebras_maquinas, incompatibilidade = st.tabs(["Carregar Planilha", "KPI's", "Tabelas", "Quebra de Maquinas", "Incompatibilidade"])
